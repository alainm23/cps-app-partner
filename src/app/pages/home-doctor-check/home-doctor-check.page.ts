...
export class HomeDoctorCheckPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.storage.getParams_2 ().then (data => {
      ...
      if (params.type === 'canceled') {
        this.subscription_1 = this.database.getHomeDoctorCanceladoByKey (params.id).subscribe (data => {
        ...
        this.subscription_1 = this.database.getHomeDoctorByKey (params.id).subscribe ((data: any) => {
          this.home_injection = data;
          loading.dismiss ();
        });

        this.subscription_2 = this.database.getHomeDoctorObservations (params.id).subscribe ((data: any) => {
          if (data) {
  ...
  async cancel () {
    const confirm = await this.alertController.create({
      header: '¿Está seguro que desea cancelar?',
      message: '',
      inputs: [
        {
          name: 'message',
